...
const getItems = async (
  ...
  const res = await fetch(
    `${process.env.API_URI}/api/v1/item?${search ? "search=" + search : ""}${
...
export default async function page({ searchParams }: IProps) {
  const items: Array<IItem> = await getItems(
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-4 bg-white p-8">
            ...
            <div className="flex flex-col gap-2">
              <h2 className="font-medium">Women</h2>
        ...
        <div className="flex flex-col gap-4 w-full">
          <div className="flex justify-between items-center">
            {searchParams.search && (
              <h1>
                Search result for{" "}
                <span className="text-primary">‘{searchParams.search}’</span>
              </h1>
            )}
